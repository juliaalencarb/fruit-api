...
// middlewares -> should always be on the top of the file.
app.use(logger);
...
app.post("/fruits", (req, res) => {
    ...
    console.log(fruit) // it's undefined since fruit is no found
    if (fruit != undefined) {
        res.status(409).send();
    }
    else {
        let newFruitId = createUniqueId()
...
app.delete('/fruits/:fruitName', (req, res, next) => {
    const fruitName = req.params.fruitName.toLowerCase()
    const index = fruits.findIndex((fruit) => fruit.name.toLowerCase() === fruitName)
    if (index > -1) {
...
function createUniqueId() {
    let allFruitsIds = [];
    fruits.forEach(function(fruit) {allFruitsIds.push(fruit.id)})
    let highestId = Math.max(...allFruitsIds)
    return highestId + 1
}
